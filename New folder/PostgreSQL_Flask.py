...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:admin@localhost/project1'
    ...
    if request.method == 'POST':
        try:
            if not request.form['dep_name']:
                ...
                #return render_template('deptable.html', dep_data=dep_data)
                return redirect(url_for('index'))
        except Exception as e:
            return '<h2 style = "text-align: center" >This Department Is Already Present In Table Add New Department.</h2>'
            # return "This Department Is Already Present In Table Add New Department."
    ...
    if request.method == 'POST':
        try:
            if not request.form['emp_name'] or not request.form['emp_dep_id']:
                ...
                #return render_template('emptable.html', emp_data=emp_data)
                return redirect(url_for('index'))
        except Exception as e:
            return e
    ...
    if request.method == 'POST':
        try:
            if not request.form['old_emp_id'] or not request.form['new_dep_id']:
                ...
                update = Employee.query.filter_by(emp_id=old_emp_id).update(dict(dep_id=new_dep_id))
                db.session.commit()
                #emp = Employee.query.filter_by(emp_id=old_emp_id).first()
                """
                emp = Employee.query.order_by(emp_id=old_emp_id).first()
